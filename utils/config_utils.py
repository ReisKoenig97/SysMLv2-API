...
def save_config(config_file_path, config_data_dict):
    ...
    try:
        ...
        if save_json(config_file_path, config_data_dict):
            print(f"Config file '{config_file_path}' saved successfully. ({__name__}.save_config)")
            return True
        else:
            return False
    except IOError as e:
...
def load_config(config_file_path="config/default_config.json"):
    ...
    if not os.path.exists(config_file_path):
        ...
        if save_config(config_file_path, default_config_dict):
            print(f"Cannot find Default config ... Created new file at '{config_file_path}'.")
            return default_config_dict
        else:
            print(f"Failed to create default config at '{config_file_path}'.")
            return None
    ...
    try:
        ...
        if config_data is not None:
            ...
            print(f"Error loading config file '{config_file_path}' in {__name__}.load_config()")
            return None
    except Exception as e:
        print(f"Error reading the config file '{config_file_path}': {e} raised in {__name__}.load_config()")
        return None
